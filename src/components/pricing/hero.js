...
const Hero = ({title, description, fimage, description2, pageName}) => {  
  ...
  return (
    <>
        <div>
            <div className={`max-w-1366px mx-auto ${!!pageName && pageName === "aboutUs"?("mt-70px"):("mt-20 lg:mt-36 xl:mt-44")} pt-67px pb-67px pl-20px pr-20px md:pl-40px md:pr-40px lg:pl-60px lg:pr-60px xl:pl-80px xl:pr-80px 2xl:pl-80px 2xl:pr-80px`}>
               <div className={`grid ${!!pageName && pageName === "aboutUs"?("md:grid-cols-3 lg:grid-cols-4 gap-4"):("grid-cols-2 gap-6")} items-end`}>
                   <div className={`${!!pageName && pageName === "aboutUs"?("md:col-span-2 lg:col-span-3"):("")}`}>
                        <h1 className="text-30px md:text-30px xl:text-60px new-line text-blue font-proximaBold mb-26px">{title}</h1>
